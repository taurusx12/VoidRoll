...
async function pityCreateCard(userId, character) {
  ...
  return prisma.userCard.create({
    ...
    return prisma.userCard.create({ data: { id: `card_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`, userId, characterId: character.id, power: Number(character.basePower || 3000) } });
  });
}
...
async function tpPackChoices() {
  const picks = await tpDailyBannerChars();
  const choices = [{ name: 'REMOVED OLD ALL FEATURED', value: 'daily-secret' }];
...
// ===== PACK ULTRA CLEAN FINAL =====
// Runs before every other pack/banner/autocomplete handler.
// Fixes required userCard.id and removes "All 4 Featured" permanently.

const ulPackPityMemory = new Map();

function ulClean(name = '') {
  return String(name || '')
    .replace(/\s*\([^)]*\)\s*/g, ' ')
    .replace(/\b(true power|base|elite|prime|final arc|mythic form|awakened|battle ready|divine form|support|training|limit break|domain form|early arc|transcendent|ultimate|form|mode|arc|version)\b/ig, ' ')
    .replace(/\s+/g, ' ')
...
function ulCardId() {
  return `ucard_${Date.now()}_${Math.random().toString(36).slice(2, 12)}_${Math.random().toString(36).slice(2, 7)}`;
}
...
function ulPityKey(characterId) {
  return `UltraPity_${String(characterId || 'x').replace(/[^a-zA-Z0-9]/g, '')}`;
}

function ulTraitGet(trait = '', key = '') {
  const safe = String(key).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const m = String(trait || '').match(new RegExp(`${safe}:(\\d+)`, 'i'));
  return m ? Number(m[1] || 0) : null;
}

function ulTraitSet(trait = '', key = '', value = 0) {
  const safe = String(key).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const clean = String(trait || '').replace(new RegExp(`\\s*\\|?\\s*${safe}:\\d+`, 'ig'), '').trim();
  return `${clean}${clean ? ' | ' : ''}${key}:${Math.max(0, Number(value || 0))}`;
}

async function ulGetPity(userId, characterId) {
  const key = ulPityKey(characterId);
  const memKey = `${userId}:${key}`;
  const user = await prisma.user.findUnique({ where: { id: userId } }).catch(() => null);
  const traitValue = ulTraitGet(user?.trait, key);
  if (traitValue !== null) return traitValue;
  return ulPackPityMemory.get(memKey) || 0;
}

async function ulSavePity(userId, characterId, value) {
  const key = ulPityKey(characterId);
  const memKey = `${userId}:${key}`;
  ulPackPityMemory.set(memKey, value);
  const user = await prisma.user.findUnique({ where: { id: userId } }).catch(() => null);
  if (!user) return;
  await prisma.user.update({
    where: { id: userId },
    data: { trait: ulTraitSet(user.trait, key, value) }
  }).catch(() => null);
}

async function ulDailySecrets() {
  ...
  const seen = new Set();
  for (const c of all) {
    const key = `${ulClean(c.name).toLowerCase()}::${String(c.anime || '').toLowerCase()}`;
    if (seen.has(key)) continue;
    seen.add(key);
    unique.push(c);
  }

  const seed = ulDaySeed();
  const picks = [];
...
async function ulPackChoices() {
  const picks = await ulDailySecrets();
  // IMPORTANT: no "All 4 Featured" choice.
  return picks.map(c => ({
    name: `Rate Up: ${ulClean(c.name)}`,
    value: `ul:${c.id}`
  })).slice(0, 25);
}

async function ulPackAutocomplete(i) {
  if (i.commandName !== 'pack') return false;
  const focused = String(i.options.getFocused() || '').toLowerCase();
  const choices = await ulPackChoices();
  const filtered = choices.filter(c => c.name.toLowerCase().includes(focused)).slice(0, 25);
  await i.respond(filtered).catch(() => null);
  return true;
}

async function ulGetSelected(value) {
  const choices = await ulPackChoices();
  let id = null;

  if (value && String(value).startsWith('ul:')) id = String(value).replace('ul:', '');
  else if (value && String(value).startsWith('featured:')) id = String(value).replace('featured:', '');
  else id = choices[0]?.value?.replace('ul:', '');
...
async function ulPickByRarity(rarity) {
  const chars = await prisma.character.findMany({ where: { active: true, rarity }, take: 400 }).catch(() => []);
  if (chars.length) return chars[Math.floor(Math.random() * chars.length)];
  return prisma.character.findFirst({ where: { active: true }, orderBy: { basePower: 'desc' } }).catch(() => null);
}

async function ulCreateCard(userId, character) {
  // No fallback without id. Schema requires id.
...
async function ulBanner(i) {
  const picks = await ulDailySecrets();
  const ends = Math.floor(((ulDaySeed() + 1) * 86400000) / 1000);
  const lines = [];
  for (let idx = 0; idx < picks.length; idx++) {
    const c = picks[idx];
    const pity = await ulGetPity(i.user.id, c.id);
    lines.push(`${idx + 1}. **${ulClean(c.name)}** • ${c.anime} • SECRET • Pity **${pity}/50**`);
  }
  const embed = new EmbedBuilder()
    .setTitle('Daily SECRET Banner')
    .setDescription(
...
async function ulPack(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);

  const selected = await ulGetSelected(i.options.getString('banner', true));
  if (!selected) return i.editReply('Select a valid Rate Up character from /pack banner.');

  const user = await prisma.user.findUnique({ where: { id: i.user.id } });
  const cost = 4000;
  if ((user?.tokens || 0) < cost) {
    return i.editReply(`You need **${ulMoney(cost)} Tokens** for 10 pulls.\nYou have **${ulMoney(user?.tokens || 0)} Tokens**.`);
  }

  await prisma.user.update({ where: { id: i.user.id }, data: { tokens: { decrement: cost } } }).catch(() => null);

  let pity = await ulGetPity(i.user.id, selected.id);
  const before = pity;
  const rarities = ['RARE', 'RARE', 'RARE', 'EPIC', 'EPIC', 'EPIC', 'LEGENDARY', 'LEGENDARY', 'MYTHIC', 'DIVINE'];
  ...
  for (let n = 0; n < 10; n++) {
    ...
    if (rarities[n] === 'SECRET') {
      character = selected;
      gotSecret = true;
      pity = 0; // reset immediately if selected character appears by luck or hard pity
    } else {
      character = await ulPickByRarity(rarities[n]);
    }

    if (!character) continue;
    const card = await ulCreateCard(i.user.id, character);

    lines.push(`${n + 1}. ${ulEmoji(character.rarity)} **${ulClean(character.name)}** • ${character.anime} • ${character.rarity} • PWR ${ulMoney(card.power || character.basePower)}`);

    if (String(character.rarity).toUpperCase() === 'SECRET' || embeds.length < 2) {
      const embed = new EmbedBuilder()
        .setTitle(`${ulEmoji(character.rarity)} ${ulClean(character.name)}`)
        .setDescription(
          `Anime: **${character.anime}**\n` +
          `Rarity: **${character.rarity}**\n` +
          `Power: **${ulMoney(card.power || character.basePower)}**\n` +
          `Selected Rate Up: **${ulClean(selected.name)}**\n` +
          `This character pity: **${pity}/50**\n` +
          (String(character.rarity).toUpperCase() === 'SECRET' ? `🔥 SECRET obtained. ${ulClean(selected.name)} pity reset.` : `Guaranteed at 50 pulls for this selected character.`)
        )
        .setColor(String(character.rarity).toUpperCase() === 'SECRET' ? 0xe74c3c : 0x9b59b6);
      if (character.imageUrl) embed.setImage(character.imageUrl);
      embeds.push(embed);
  ...
  await ulSavePity(i.user.id, selected.id, pity);

  return i.editReply({
    content:
      (`**PACK x10**\n` +
      `Selected Rate Up: **${ulClean(selected.name)}**\n` +
      `Cost: **${ulMoney(cost)} Tokens**\n` +
      `This character pity: **${before}/50 → ${pity}/50**\n` +
      (gotSecret ? `🔥 SECRET pulled! ${ulClean(selected.name)} pity reset.\n` : ``) +
      `\n${lines.join('\n')}\n\n` +
      `Tokens left: **${ulMoney((user?.tokens || 0) - cost)}**`).slice(0, 1900),
    embeds: embeds.slice(0, 5)
  });
}

async function ulHandler(i, userId, commandName) {
  if (commandName === 'banner') return ulBanner(i);
  if (commandName === 'pack') return ulPack(i);
  return false;
}
...
// ===== BANNER PITY BY NAME HARD FIX =====
// Fixes mismatch where /pack shows 10/50 but /banner still shows 0/50.
// Pity is now stored by clean character name + anime, not raw character ID,
// so Makima stays synced even if multiple DB IDs/variants exist.

function bpnClean(name = '') {
  return String(name || '')
    .replace(/\s*\([^)]*\)\s*/g, ' ')
    .replace(/\b(true power|base|elite|prime|final arc|mythic form|awakened|battle ready|divine form|support|training|limit break|domain form|early arc|transcendent|ultimate|form|mode|arc|version)\b/ig, ' ')
    .replace(/\s+/g, ' ')
...
function bpnSafeKeyText(v = '') {
  return String(v || '').toLowerCase().replace(/[^a-z0-9]+/g, '');
}
function bpnPityKey(character) {
  const name = bpnSafeKeyText(bpnClean(character?.name || ''));
  const anime = bpnSafeKeyText(character?.anime || '');
  return `PackPityName_${name}_${anime}`;
}
function bpnOldIdKey(characterId) {
  return `PackPity_${String(characterId || 'x').replace(/[^a-zA-Z0-9]/g, '')}`;
}
function bpnTraitGet(trait = '', key = '') {
  const safe = String(key).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const m = String(trait || '').match(new RegExp(`${safe}:(\\d+)`, 'i'));
  return m ? Number(m[1] || 0) : 0;
}
function bpnTraitSet(trait = '', key = '', value = 0) {
  const safe = String(key).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const clean = String(trait || '').replace(new RegExp(`\\s*\\|?\\s*${safe}:\\d+`, 'ig'), '').trim();
  return `${clean}${clean ? ' | ' : ''}${key}:${Math.max(0, Number(value || 0))}`;
}
async function bpnGetPity(userId, character) {
  const user = await prisma.user.findUnique({ where: { id: userId } }).catch(() => null);
  const trait = user?.trait || '';
  const nameKey = bpnPityKey(character);
  const idKey = bpnOldIdKey(character?.id);
  const byName = bpnTraitGet(trait, nameKey);
  const byId = bpnTraitGet(trait, idKey);
  return Math.max(byName, byId);
}
async function bpnSavePity(userId, character, value) {
  const user = await prisma.user.findUnique({ where: { id: userId } }).catch(() => null);
  if (!user) return;
  const v = Math.max(0, Number(value || 0));
  let trait = user.trait || '';
  trait = bpnTraitSet(trait, bpnPityKey(character), v);
  trait = bpnTraitSet(trait, bpnOldIdKey(character?.id), v);
  await prisma.user.update({ where: { id: userId }, data: { trait } }).catch(() => null);
}
function bpnIsExactMakima(c) {
  const clean = bpnClean(c?.name || '').toLowerCase();
  return clean === 'makima';
}
async function bpnAllSecrets() {
  const all = await prisma.character.findMany({
    where: { active: true, rarity: 'SECRET' },
    orderBy: { basePower: 'desc' },
    take: 800
  ...
  const unique = [];
  for (const c of all) {
    const key = `${bpnClean(c.name).toLowerCase()}::${String(c.anime || '').toLowerCase()}`;
    if (seen.has(key)) continue;
    seen.add(key);
    unique.push(c);
  }
...
async function bpnBannerChars() {
  if (typeof cm4EndMs !== 'undefined' && Date.now() < cm4EndMs) {
    const pool = await bpnAllSecrets();
    if (!pool.length) return [];
    const makima = pool.find(bpnIsExactMakima);
    const others = pool.filter(c => !bpnIsExactMakima(c));
    const picks = [];
    const seedBase = typeof cm4StartMs !== 'undefined' ? cm4StartMs : Date.now();
    const seed = Math.floor(seedBase / 86400000);
    const start = (seed * 47 + 29) % Math.max(1, others.length);
    const steps = [0, 31, 73, 127, 181, 233];
...
async function bpnChoices() {
  const picks = await bpnBannerChars();
  return picks.map(c => ({
    name: `Rate Up: ${bpnClean(c.name)}`,
    value: `bpn:${c.id}`
  })).slice(0, 25);
}
async function bpnAutocomplete(i) {
  if (i.commandName !== 'pack') return false;
  const focused = String(i.options.getFocused() || '').toLowerCase();
  const choices = await bpnChoices();
  const filtered = choices.filter(c => c.name.toLowerCase().includes(focused)).slice(0, 25);
  await i.respond(filtered.length ? filtered : choices).catch(() => null);
  return true;
}
async function bpnSelected(value) {
  const choices = await bpnChoices();
  let id = null;
  const v = String(value || '');
  if (v.startsWith('bpn:')) id = v.replace('bpn:', '');
  else if (v.startsWith('bps:')) id = v.replace('bps:', '');
  else if (v.startsWith('cm4:')) id = v.replace('cm4:', '');
  else if (v.startsWith('featured:')) id = v.replace('featured:', '');
  else if (v.startsWith('mb:')) id = v.replace('mb:', '');
  else if (v.startsWith('br:')) id = v.replace('br:', '');
  ...
  else id = choices[0]?.value?.replace('bpn:', '');
...
async function bpnPickByRarity(rarity) {
  const chars = await prisma.character.findMany({ where: { active: true, rarity }, take: 500 }).catch(() => []);
  if (chars.length) return chars[Math.floor(Math.random() * chars.length)];
  return prisma.character.findFirst({ where: { active: true }, orderBy: { basePower: 'desc' } }).catch(() => null);
}
function bpnId(prefix = 'pack') {
  return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 11)}_${Math.random().toString(36).slice(2, 7)}`;
}
function bpnSerial() {
  return Math.floor((Date.now() + Math.floor(Math.random() * 1000000)) % 2000000000);
}
async function bpnCreateCard(userId, character) {
  for (let attempt = 0; attempt < 6; attempt++) {
    try {
      return await prisma.userCard.create({
        data: {
...
function bpnStatsText(card, c) {
  if (typeof uxStatsText === 'function') return uxStatsText(card, c);
  if (typeof viStatsText === 'function') return viStatsText(card, c);
  if (typeof uiStatsText === 'function') return uiStatsText(card, c);
  return `Power: **${bpnMoney(card.power || c.basePower || 0)}**`;
}
async function bpnBanner(i) {
  const picks = await bpnBannerChars();
  const ends = bpnEnds();
  const lines = [];
  ...
  for (let idx = 0; idx < picks.length; idx++) {
    const c = picks[idx];
    const pity = await bpnGetPity(i.user.id, c);
    lines.push(`${idx + 1}. **${bpnClean(c.name)}** • ${c.anime} • SECRET • Pity **${pity}/50**`);
  }
...
async function bpnPack(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);

  const selected = await bpnSelected(i.options.getString('banner', true));
  if (!selected) return i.editReply('Select a valid Rate Up character from /pack banner.');
  ...
  if ((user?.tokens || 0) < cost) {
    return i.editReply(`You need **${bpnMoney(cost)} Tokens** for 10 pulls.\nYou have **${bpnMoney(user?.tokens || 0)} Tokens**.`);
  }

  await prisma.user.update({ where: { id: i.user.id }, data: { tokens: { decrement: cost } } }).catch(() => null);

  let pity = await bpnGetPity(i.user.id, selected);
  const before = pity;
  const rarities = ['RARE', 'RARE', 'RARE', 'EPIC', 'EPIC', 'EPIC', 'LEGENDARY', 'LEGENDARY', 'MYTHIC', 'DIVINE'];
  ...
  for (let n = 0; n < 10; n++) {
    ...
    if (rarities[n] === 'SECRET') {
      ...
      character = await bpnPickByRarity(rarities[n]);
    }

    if (!character) continue;
    const card = await bpnCreateCard(i.user.id, character);
    const power = Number(card.power || character.basePower || 0);

    lines.push(`${n + 1}. ${bpnEmoji(character.rarity)} **${bpnClean(character.name)}** • ${character.anime} • ${character.rarity} • PWR ${bpnMoney(power)}`);

    const embed = new EmbedBuilder()
      .setTitle(`${n + 1}. ${bpnEmoji(character.rarity)} ${bpnClean(character.name)}`)
      .setDescription(
        `Anime: **${character.anime}**\n` +
        `Rarity: **${character.rarity}**\n` +
        `Power: **${bpnMoney(power)}**\n` +
        `${bpnStatsText(card, character)}\n` +
        `Selected Rate Up: **${bpnClean(selected.name)}**\n` +
        `This character pity: **${pity}/50**` +
        (String(character.rarity).toUpperCase() === 'SECRET' ? `\n🔥 SECRET obtained. Pity reset.` : ``)
      )
      .setColor(String(character.rarity).toUpperCase() === 'SECRET' ? 0xe74c3c : 0x9b59b6);
  ...
  await bpnSavePity(i.user.id, selected, pity);

  return i.editReply({
    content:
      (`**PACK x10**\n` +
      `Selected Rate Up: **${bpnClean(selected.name)}**\n` +
      `Cost: **${bpnMoney(cost)} Tokens**\n` +
      `This character pity: **${before}/50 → ${pity}/50**\n` +
      (gotSelectedSecret ? `🔥 SECRET pulled! Pity reset.\n` : ``) +
      `\n${lines.join('\n')}\n\n` +
      `Tokens left: **${bpnMoney((user?.tokens || 0) - cost)}**`).slice(0, 1900),
    embeds: embeds.slice(0, 10)
  });
}
async function bpnHandler(i, userId, commandName) {
  if (commandName === 'banner') return bpnBanner(i);
  if (commandName === 'pack') return bpnPack(i);
  return false;
}
// ===== END BANNER PITY BY NAME HARD FIX =====


// ===== ABS2 BANNER PACK PITY SYNC PATCH =====
// Hard override: /pack and /banner share pity by clean name+anime AND IDs.

function abs2Clean(name = '') {
  return String(name || '').replace(/\s*\([^)]*\)\s*/g, ' ').replace(/\b(true power|base|elite|prime|final arc|mythic form|awakened|battle ready|divine form|support|training|limit break|domain form|early arc|transcendent|ultimate|form|mode|arc|version)\b/ig, ' ').replace(/\s+/g, ' ').trim();
}
function abs2Money(n) { return typeof money === 'function' ? money(n) : Number(n || 0).toLocaleString('en-US'); }
function abs2Emoji(r) { return typeof rarityEmoji === 'function' ? rarityEmoji(r) : '⭐'; }
function abs2Safe(v = '') { return String(v || '').toLowerCase().replace(/[^a-z0-9]+/g, ''); }
function abs2DisplayKey(c) { return `BannerPity_${abs2Safe(abs2Clean(c?.name || ''))}_${abs2Safe(c?.anime || '')}`; }
function abs2IdKey(id) { return `BannerPityId_${String(id || 'x').replace(/[^a-zA-Z0-9]/g, '')}`; }
function abs2OldIdKey(id) { return `PackPity_${String(id || 'x').replace(/[^a-zA-Z0-9]/g, '')}`; }
function abs2TraitGet(trait = '', key = '') {
  const safe = String(key).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const m = String(trait || '').match(new RegExp(`${safe}:(\\d+)`, 'i'));
  return m ? Number(m[1] || 0) : 0;
}
function abs2TraitSet(trait = '', key = '', value = 0) {
  const safe = String(key).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const clean = String(trait || '').replace(new RegExp(`\\s*\\|?\\s*${safe}:\\d+`, 'ig'), '').trim();
  return `${clean}${clean ? ' | ' : ''}${key}:${Math.max(0, Number(value || 0))}`;
}
async function abs2GetPity(userId, c) {
  const user = await prisma.user.findUnique({ where: { id: userId } }).catch(() => null);
  const trait = user?.trait || '';
  return Math.max(
    abs2TraitGet(trait, abs2DisplayKey(c)),
    abs2TraitGet(trait, abs2IdKey(c?.id)),
    abs2TraitGet(trait, abs2OldIdKey(c?.id))
  );
}
async function abs2SavePity(userId, c, value) {
  const user = await prisma.user.findUnique({ where: { id: userId } }).catch(() => null);
  if (!user) return;
  const v = Math.max(0, Number(value || 0));
  let trait = user.trait || '';
  trait = abs2TraitSet(trait, abs2DisplayKey(c), v);
  trait = abs2TraitSet(trait, abs2IdKey(c?.id), v);
  trait = abs2TraitSet(trait, abs2OldIdKey(c?.id), v);
  await prisma.user.update({ where: { id: userId }, data: { trait } }).catch(() => null);
}
function abs2IsMakima(c) { return abs2Clean(c?.name || '').toLowerCase() === 'makima'; }
async function abs2Secrets() {
  const all = await prisma.character.findMany({ where: { active: true, rarity: 'SECRET' }, orderBy: { basePower: 'desc' }, take: 1000 }).catch(() => []);
  const seen = new Set(), unique = [];
  for (const c of all) {
    const key = `${abs2Clean(c.name).toLowerCase()}::${String(c.anime || '').toLowerCase()}`;
    if (seen.has(key)) continue;
    seen.add(key);
    unique.push(c);
  }
...
async function abs2BannerChars() {
  const pool = await abs2Secrets();
  if (!pool.length) return [];

  if (typeof cm4EndMs !== 'undefined' && Date.now() < cm4EndMs) {
    const makima = pool.find(abs2IsMakima);
    const others = pool.filter(c => !abs2IsMakima(c));
    const seedBase = typeof cm4StartMs !== 'undefined' ? cm4StartMs : Date.now();
    const seed = Math.floor(seedBase / 86400000);
    const start = (seed * 47 + 29) % Math.max(1, others.length);
    const steps = [0, 31, 73, 127, 181, 233];
...
async function abs2Choices() {
  const picks = await abs2BannerChars();
  return picks.map(c => ({ name: `Rate Up: ${abs2Clean(c.name)}`, value: `abs2:${c.id}` })).slice(0, 25);
}
async function abs2Autocomplete(i) {
  if (i.commandName !== 'pack') return false;
  const focused = String(i.options.getFocused() || '').toLowerCase();
  const choices = await abs2Choices();
  const filtered = choices.filter(c => c.name.toLowerCase().includes(focused)).slice(0, 25);
  await i.respond(filtered.length ? filtered : choices).catch(() => null);
  return true;
}
async function abs2Selected(value) {
  const choices = await abs2Choices();
  let id = null;
  const v = String(value || '');
  if (v.startsWith('abs2:')) id = v.replace('abs2:', '');
  else if (v.includes(':')) id = v.split(':').slice(1).join(':');
  else id = choices[0]?.value?.replace('abs2:', '');
  const c = id ? await prisma.character.findUnique({ where: { id } }).catch(() => null) : null;
  if (c && c.active && String(c.rarity || '').toUpperCase() === 'SECRET') return c;
  return null;
}
async function abs2PickByRarity(rarity) {
  const chars = await prisma.character.findMany({ where: { active: true, rarity }, take: 500 }).catch(() => []);
  if (chars.length) return chars[Math.floor(Math.random() * chars.length)];
  return prisma.character.findFirst({ where: { active: true }, orderBy: { basePower: 'desc' } }).catch(() => null);
}
function abs2CardId(prefix = 'pack') { return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2, 11)}_${Math.random().toString(36).slice(2, 7)}`; }
function abs2Serial() { return Math.floor((Date.now() + Math.floor(Math.random() * 1000000)) % 2000000000); }
async function abs2CreateCard(userId, character) {
  for (let attempt = 0; attempt < 6; attempt++) {
    try {
      return await prisma.userCard.create({ data: { id: abs2CardId('pack'), serial: abs2Serial(), userId: String(userId), characterId: String(character.id), power: Number(character.basePower || 1000) } });
    } catch (err) {
      if (attempt === 5) throw err;
    }
  }
}
function abs2StatsText(card, c) {
  if (typeof uxStatsText === 'function') return uxStatsText(card, c);
  if (typeof viStatsText === 'function') return viStatsText(card, c);
  if (typeof uiStatsText === 'function') return uiStatsText(card, c);
  return `Power: **${abs2Money(card.power || c.basePower || 0)}**`;
}
async function abs2Banner(i) {
  const picks = await abs2BannerChars();
  const ends = abs2Ends();
  const lines = [];
  for (let idx = 0; idx < picks.length; idx++) {
    const c = picks[idx];
    const pity = await abs2GetPity(i.user.id, c);
    lines.push(`${idx + 1}. **${abs2Clean(c.name)}** • ${c.anime} • SECRET • Pity **${pity}/50**`);
  }
  const embed = new EmbedBuilder().setTitle('Daily SECRET Banner').setDescription(
    `Choose one exact Rate Up in **/pack banner**.\n` +
    `Each character has its own pity.\n` +
    `10 pulls: **4,000 Tokens**\n` +
    `Guaranteed selected SECRET: **50 pulls / 20,000 Tokens**\n` +
...
async function abs2Pack(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);
  const selected = await abs2Selected(i.options.getString('banner', true));
  if (!selected) return i.editReply('Select a valid Rate Up character from /pack banner.');

  const user = await prisma.user.findUnique({ where: { id: i.user.id } });
  const cost = 4000;
  if ((user?.tokens || 0) < cost) return i.editReply(`You need **${abs2Money(cost)} Tokens** for 10 pulls.\nYou have **${abs2Money(user?.tokens || 0)} Tokens**.`);

  await prisma.user.update({ where: { id: i.user.id }, data: { tokens: { decrement: cost } } }).catch(() => null);

  let pity = await abs2GetPity(i.user.id, selected);
  const before = pity;
  const rarities = ['RARE', 'RARE', 'RARE', 'EPIC', 'EPIC', 'EPIC', 'LEGENDARY', 'LEGENDARY', 'MYTHIC', 'DIVINE'];
  ...
  for (let n = 0; n < 10; n++) {
    ...
    if (rarities[n] === 'SECRET') {
      ...
      character = await abs2PickByRarity(rarities[n]);
    }
    if (!character) continue;

    const card = await abs2CreateCard(i.user.id, character);
    const power = Number(card.power || character.basePower || 0);
    lines.push(`${n + 1}. ${abs2Emoji(character.rarity)} **${abs2Clean(character.name)}** • ${character.anime} • ${character.rarity} • PWR ${abs2Money(power)}`);

    const embed = new EmbedBuilder().setTitle(`${n + 1}. ${abs2Emoji(character.rarity)} ${abs2Clean(character.name)}`).setDescription(
      `Anime: **${character.anime}**\n` +
      `Rarity: **${character.rarity}**\n` +
      `Power: **${abs2Money(power)}**\n` +
      `${abs2StatsText(card, character)}\n` +
      `Selected Rate Up: **${abs2Clean(selected.name)}**\n` +
      `This character pity: **${pity}/50**` +
      (String(character.rarity).toUpperCase() === 'SECRET' ? `\n🔥 SECRET obtained. Pity reset.` : ``)
    ).setColor(String(character.rarity).toUpperCase() === 'SECRET' ? 0xe74c3c : 0x9b59b6);
    if (character.imageUrl) embed.setImage(character.imageUrl);
  ...
  await abs2SavePity(i.user.id, selected, pity);

  return i.editReply({
    content: (`**PACK x10**\nSelected Rate Up: **${abs2Clean(selected.name)}**\nCost: **${abs2Money(cost)} Tokens**\nThis character pity: **${before}/50 → ${pity}/50**\n${gotSelectedSecret ? '🔥 SECRET pulled! Pity reset.\n' : ''}\n${lines.join('\n')}\n\nTokens left: **${abs2Money((user?.tokens || 0) - cost)}**`).slice(0, 1900),
    embeds: embeds.slice(0, 10)
  });
}
async function abs2Handler(i, userId, commandName) {
  if (commandName === 'banner') return abs2Banner(i);
  if (commandName === 'pack') return abs2Pack(i);
  return false;
}
...
// ===== FINAL GIFT + TRADE COMMANDS FIX =====
// Fixes:
// 1) /gift-character and /give-character not showing.
// 2) /trade-offer failed / trade accept issues.
// 3) Trade ID confusion by also adding buttons Accept / Decline.
// 4) Safer token update with BigInt/Number fallback.
// 5) No ID needed for character names.

const finalTradeMemory = global.finalTradeMemory || new Map();
global.finalTradeMemory = finalTradeMemory;

function ftClean(name = '') {
  return String(name || '')
    .replace(/\s*\([^)]*\)\s*/g, ' ')
    .replace(/\b(true power|base|elite|prime|final arc|mythic form|awakened|battle ready|divine form|support|training|limit break|domain form|early arc|transcendent|ultimate|form|mode|arc|version)\b/ig, ' ')
    .replace(/\s+/g, ' ')
...
function ftLevel(card) {
  const m = String(card?.trait || '').match(/Level:(\d+)/i);
  return Math.max(1, Math.min(99, Number(card?.level || (m ? m[1] : 1) || 1)));
}
function ftStars(card) {
  const m = String(card?.trait || '').match(/Stars:(\d+)/i);
  return Math.max(0, Math.min(10, Number(m ? m[1] : 0)));
}
function ftStarsText(stars) {
  stars = Math.max(0, Math.min(10, Number(stars || 0)));
  return stars > 0 ? '★'.repeat(stars) : '0';
}
function ftRole(c) {
...
async function ftFindOwnedCard(userId, query) {
  const tokens = ftNorm(query).split(/\s+/).filter(Boolean);
  const cards = await prisma.userCard.findMany({
    where: { userId: String(userId) },
    include: { character: true },
    orderBy: [{ power: 'desc' }, { id: 'desc' }],
  ...
  const scored = cards.map(card => {
    const c = card.character || {};
    const cleanName = ftNorm(ftClean(c.name || ''));
    const rawName = ftNorm(c.name || '');
    const anime = ftNorm(c.anime || '');
    const full = `${cleanName} ${rawName} ${anime}`;

    let score = 0;
    for (const t of tokens) {
      if (cleanName.includes(t)) score += 120;
      if (rawName.includes(t)) score += 90;
      if (anime.includes(t)) score += 20;
      if (full.includes(t)) score += 35;
    }
    if (tokens.length && tokens.every(t => full.includes(t))) score += 220;
    if (cleanName === ftNorm(query)) score += 500;
...
function ftCardLine(card) {
  const c = card.character || {};
  return `${ftEmoji(c.rarity)} **${ftClean(c.name)}** • ${c.anime}\n` +
    `Rarity: **${c.rarity}** | Power: **${ftMoney(card.power || c.basePower || 0)}** | Level: **${ftLevel(card)}/99** | Stars: **${ftStarsText(ftStars(card))}** | Role: **${ftRole(c)}**`;
}
async function ftEnsureUser(userId) {
  let user = await prisma.user.findUnique({ where: { id: String(userId) } }).catch(() => null);
  if (user) return user;

  // Very safe fallback. If schema has required fields not listed here, this will fail silently,
  // but most users already exist after using the bot.
  user = await prisma.user.create({
    data: { id: String(userId), gold: 0, tokens: 0, rolls: 0 }
  }).catch(() => null);

  return user;
}
async function ftSetTokens(userId, newTokensBig) {
  let updated = await prisma.user.update({
    where: { id: String(userId) },
    data: { tokens: newTokensBig }
  }).catch(() => null);
  ...
  if (!updated) {
    updated = await prisma.user.update({
      where: { id: String(userId) },
      data: { tokens: ftNumSafe(newTokensBig) }
    }).catch(() => null);
  }

  return updated;
}
function ftTradeExpired(trade) {
  return !trade || Date.now() > Number(trade.expiresAt || 0);
}
async function ftGift(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);

  const target = i.options.getUser('user', true);
  const name = i.options.getString('name', true);

  if (!target || target.bot) return i.editReply('You cannot give characters to a bot.');
  if (target.id === i.user.id) return i.editReply('You cannot give a character to yourself.');

  await ftEnsureUser(target.id).catch(() => null);

  const card = await ftFindOwnedCard(i.user.id, name);
  if (!card) return i.editReply(`I could not find **${name}** in your inventory.`);

  const moved = await prisma.userCard.update({
    where: { id: card.id },
    data: { userId: String(target.id) }
  }).catch(err => ({ __error: err }));

  if (!moved || moved.__error) {
    return i.editReply(`Gift failed: could not move the card.\n${moved?.__error?.message ? `\`${String(moved.__error.message).slice(0, 300)}\`` : ''}`);
  }

  const c = card.character || {};
  const embed = new EmbedBuilder()
    .setTitle('Gift Complete')
    .setDescription(`${i.user} gave ${target}:\n\n${ftCardLine(card)}`)
    .setColor(0x2ecc71);
  if (c.imageUrl) embed.setImage(c.imageUrl);

  return i.editReply({ embeds: [embed] });
}
async function ftTradeOffer(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);

  const buyer = i.options.getUser('user', true);
  const name = i.options.getString('name', true);
  const priceNumber = i.options.getInteger('tokens', true);

  if (!buyer || buyer.bot) return i.editReply('You cannot trade with a bot.');
  if (buyer.id === i.user.id) return i.editReply('You cannot trade with yourself.');
  if (priceNumber <= 0) return i.editReply('Token price must be higher than 0.');

  await ftEnsureUser(buyer.id).catch(() => null);

  const card = await ftFindOwnedCard(i.user.id, name);
  if (!card) return i.editReply(`I could not find **${name}** in your inventory.`);

  const tradeId = ftTradeId();
  const trade = {
    id: tradeId,
    sellerId: String(i.user.id),
    buyerId: String(buyer.id),
  ...
  finalTradeMemory.set(tradeId, trade);
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `Buyer: ${buyer}\n\n` +
      `${ftCardLine(card)}\n\n` +
      `Price: **${ftMoney(priceNumber)} Tokens**\n` +
      `Trade ID: \`${tradeId}\`\n` +
      `Expires: <t:${Math.floor(trade.expiresAt / 1000)}:R>\n\n` +
      `Buyer can press **Accept** below or use:\n` +
      `\`/trade-accept trade_id:${tradeId}\``
    )
    .setColor(0xf1c40f);
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`ft_accept_${tradeId}`).setLabel('Accept Trade').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId(`ft_decline_${tradeId}`).setLabel('Decline').setStyle(ButtonStyle.Danger)
  );

  return i.editReply({ embeds: [embed], components: [row] });
}
async function ftCompleteTrade(i, tradeId) {
  const trade = finalTradeMemory.get(tradeId);

  if (!trade) return i.editReply('Trade not found or already completed.');
  if (ftTradeExpired(trade)) {
    finalTradeMemory.delete(tradeId);
    return i.editReply('This trade expired.');
  }
  if (String(i.user.id) !== String(trade.buyerId)) {
    return i.editReply('Only the buyer can accept this trade.');
  ...
  if (!card) {
    finalTradeMemory.delete(tradeId);
    return i.editReply('Trade failed: the card no longer exists.');
  }
  if (String(card.userId) !== String(trade.sellerId)) {
    finalTradeMemory.delete(tradeId);
    return i.editReply('Trade failed: seller no longer owns this character.');
  ...
  const buyer = await ftEnsureUser(trade.buyerId);
  const seller = await ftEnsureUser(trade.sellerId);
  if (!buyer || !seller) return i.editReply('Trade failed: buyer or seller profile was not found.');

  const price = ftBig(trade.price);
  const buyerTokens = ftBig(buyer.tokens || 0);
  const sellerTokens = ftBig(seller.tokens || 0);

  if (buyerTokens < price) {
    return i.editReply(`You do not have enough Tokens.\nNeed: **${ftMoney(price)}**\nYou have: **${ftMoney(buyerTokens)}**`);
  }

  const newBuyerTokens = buyerTokens - price;
  const newSellerTokens = sellerTokens + price;

  const buyerUpdated = await ftSetTokens(trade.buyerId, newBuyerTokens);
  if (!buyerUpdated) return i.editReply('Trade failed: could not remove Tokens from buyer.');

  const sellerUpdated = await ftSetTokens(trade.sellerId, newSellerTokens);
  if (!sellerUpdated) {
    await ftSetTokens(trade.buyerId, buyerTokens).catch(() => null);
    return i.editReply('Trade failed: could not add Tokens to seller. Buyer token rollback attempted.');
  }
  ...
  if (!moved) {
    await ftSetTokens(trade.buyerId, buyerTokens).catch(() => null);
    await ftSetTokens(trade.sellerId, sellerTokens).catch(() => null);
    return i.editReply('Trade failed: could not move character. Token rollback attempted.');
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `<@${trade.buyerId}> bought from <@${trade.sellerId}>:\n\n` +
      `${ftCardLine(card)}\n\n` +
      `Price paid: **${ftMoney(price)} Tokens**\n` +
      `Buyer Tokens left: **${ftMoney(newBuyerTokens)}**`
    )
    .setColor(0x2ecc71);
  if (c.imageUrl) embed.setImage(c.imageUrl);

  return i.editReply({ embeds: [embed], components: [] }).catch(() => i.editReply({ embeds: [embed] }));
}
async function ftAccept(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);
  const tradeId = i.options.getString('trade_id', true);
  return ftCompleteTrade(i, tradeId);
}
async function ftDecline(i, tradeIdInput = null) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);

  const tradeId = tradeIdInput || i.options.getString('trade_id', true);
  const trade = finalTradeMemory.get(tradeId);

  if (!trade) return i.editReply('Trade not found or already completed.');
  if (String(i.user.id) !== String(trade.buyerId) && String(i.user.id) !== String(trade.sellerId)) {
    return i.editReply('Only buyer or seller can cancel this trade.');
  }

  finalTradeMemory.delete(tradeId);
  return i.editReply(`Trade \`${tradeId}\` has been cancelled.`);
}
async function ftTrades(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);

  const rows = [];
  for (const [id, trade] of finalTradeMemory.entries()) {
    if (ftTradeExpired(trade)) {
      finalTradeMemory.delete(id);
      continue;
    }
    if (String(trade.buyerId) !== String(i.user.id) && String(trade.sellerId) !== String(i.user.id)) continue;
    ...
    rows.push(
      `ID: \`${id}\`\n` +
      `Seller: <@${trade.sellerId}> | Buyer: <@${trade.buyerId}>\n` +
      `Character: **${card?.character ? ftClean(card.character.name) : 'Unknown'}**\n` +
      `Price: **${ftMoney(trade.price)} Tokens**\n` +
      `Expires: <t:${Math.floor(Number(trade.expiresAt) / 1000)}:R>`
    );
  }
...
async function ftButtons(i) {
  if (!i.isButton()) return false;

  if (i.customId.startsWith('ft_accept_')) {
    await i.deferReply().catch(() => null);
    const tradeId = i.customId.replace('ft_accept_', '');
    return ftCompleteTrade(i, tradeId);
  }

  if (i.customId.startsWith('ft_decline_')) {
    await i.deferReply().catch(() => null);
    const tradeId = i.customId.replace('ft_decline_', '');
    return ftDecline(i, tradeId);
  }

  return false;
}
async function ftHandler(i, userId, commandName) {
  if (commandName === 'give-character') return ftGift(i);
  if (commandName === 'gift-character') return ftGift(i);
  if (commandName === 'trade-offer') return ftTradeOffer(i);
  if (commandName === 'trade-accept') return ftAccept(i);
  if (commandName === 'trade-decline') return ftDecline(i);
  if (commandName === 'trade-cancel') return ftDecline(i);
  if (commandName === 'trades') return ftTrades(i);
  return false;
}
// ===== END FINAL GIFT + TRADE COMMANDS FIX =====


// ===== TRADE ONLY FINAL SAFE PATCH =====
// Fixes /trade-offer failed.
// This version is intentionally simple and safe:
// - /trade-offer only creates a pending memory offer. No token/user update happens there.
// - /trade-accept re-checks seller ownership + buyer tokens.
// - It uses Prisma increment/decrement first, then BigInt absolute fallback.
// - Trade also has buttons.

const safeTradeMemory = global.safeTradeMemory || new Map();
global.safeTradeMemory = safeTradeMemory;

function stoClean(name = '') {
  return String(name || '')
    .replace(/\s*\([^)]*\)\s*/g, ' ')
    .replace(/\b(true power|base|elite|prime|final arc|mythic form|awakened|battle ready|divine form|support|training|limit break|domain form|early arc|transcendent|ultimate|form|mode|arc|version)\b/ig, ' ')
    .replace(/\s+/g, ' ')
...
function stoLevel(card) {
  const m = String(card?.trait || '').match(/Level:(\d+)/i);
  return Math.max(1, Math.min(99, Number(card?.level || (m ? m[1] : 1) || 1)));
}
function stoStars(card) {
  const m = String(card?.trait || '').match(/Stars:(\d+)/i);
  return Math.max(0, Math.min(10, Number(m ? m[1] : 0)));
}
function stoStarsText(stars) {
  stars = Math.max(0, Math.min(10, Number(stars || 0)));
  return stars > 0 ? '★'.repeat(stars) : '0';
}
function stoRole(c) {
...
async function stoFindOwnedCard(userId, query) {
  const tokens = stoNorm(query).split(/\s+/).filter(Boolean);
  const cards = await prisma.userCard.findMany({
    where: { userId: String(userId) },
    include: { character: true },
    orderBy: [{ power: 'desc' }, { id: 'desc' }],
  ...
  const scored = cards.map(card => {
    const c = card.character || {};
    const cleanName = stoNorm(stoClean(c.name || ''));
    const rawName = stoNorm(c.name || '');
    const anime = stoNorm(c.anime || '');
    const full = `${cleanName} ${rawName} ${anime}`;
    ...
    if (tokens.length && tokens.every(t => full.includes(t))) score += 220;
    if (cleanName === stoNorm(query)) score += 500;
...
function stoCardLine(card) {
  const c = card.character || {};
  return `${stoEmoji(c.rarity)} **${stoClean(c.name)}** • ${c.anime}\n` +
    `Rarity: **${c.rarity}** | Power: **${stoMoney(card.power || c.basePower || 0)}** | Level: **${stoLevel(card)}/99** | Stars: **${stoStarsText(stoStars(card))}** | Role: **${stoRole(c)}**`;
}
function stoExpired(trade) {
  return !trade || Date.now() > Number(trade.expiresAt || 0);
}
async function stoGetUser(userId) {
  return prisma.user.findUnique({ where: { id: String(userId) } }).catch(() => null);
}
async function stoAddTokens(userId, amountBig) {
  // Try Prisma atomic BigInt/Number increment first.
  let updated = await prisma.user.update({
    where: { id: String(userId) },
    data: { tokens: { increment: amountBig } }
  }).catch(() => null);
  if (updated) return updated;

  updated = await prisma.user.update({
    where: { id: String(userId) },
    data: { tokens: { increment: stoNumSafe(amountBig) } }
  }).catch(() => null);
  if (updated) return updated;

  // Absolute fallback.
  const user = await stoGetUser(userId);
  if (!user) return null;
  const next = stoBig(user.tokens || 0) + amountBig;
  updated = await prisma.user.update({
    where: { id: String(userId) },
    data: { tokens: next }
  }).catch(() => null);
  ...
  return prisma.user.update({
    where: { id: String(userId) },
    data: { tokens: stoNumSafe(next) }
  }).catch(() => null);
}
async function stoRemoveTokens(userId, amountBig) {
  const user = await stoGetUser(userId);
  if (!user) return { ok: false, reason: 'profile' };

  const current = stoBig(user.tokens || 0);
  if (current < amountBig) return { ok: false, reason: 'funds', current };

  // Absolute set is safest for BigInt issues.
  const next = current - amountBig;
  ...
  if (!updated) return { ok: false, reason: 'update' };
  return { ok: true, before: current, after: next };
}
async function stoTradeOffer(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);
  ...
  if (!buyer || buyer.bot) return i.editReply('You cannot trade with a bot.');
  if (buyer.id === i.user.id) return i.editReply('You cannot trade with yourself.');
  if (!priceNumber || priceNumber <= 0) return i.editReply('Token price must be higher than 0.');

  const card = await stoFindOwnedCard(i.user.id, name);
  if (!card) return i.editReply(`I could not find **${name}** in your inventory.`);

  const tradeId = stoTradeId();
  const trade = {
    id: tradeId,
    sellerId: String(i.user.id),
    buyerId: String(buyer.id),
  ...
  safeTradeMemory.set(tradeId, trade);
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      ...
      `Buyer: ${buyer}\n\n` +
      `${stoCardLine(card)}\n\n` +
      `Price: **${stoMoney(priceNumber)} Tokens**\n` +
      `Trade ID: \`${tradeId}\`\n` +
      `Expires: <t:${Math.floor(trade.expiresAt / 1000)}:R>\n\n` +
      `${buyer} press **Accept Trade** or use:\n` +
      `\`/trade-accept trade_id:${tradeId}\``
    )
    .setColor(0xf1c40f);
  if (c.imageUrl) embed.setImage(c.imageUrl);
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`sto_accept_${tradeId}`).setLabel('Accept Trade').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId(`sto_decline_${tradeId}`).setLabel('Decline').setStyle(ButtonStyle.Danger)
  );

  return i.editReply({ embeds: [embed], components: [row] });
}
async function stoComplete(i, tradeId) {
  const trade = safeTradeMemory.get(tradeId);

  if (!trade) return i.editReply('Trade not found or already completed.');
  if (stoExpired(trade)) {
    safeTradeMemory.delete(tradeId);
    return i.editReply('This trade expired.');
  }
  if (String(i.user.id) !== String(trade.buyerId)) {
    return i.editReply('Only the buyer can accept this trade.');
  ...
  const seller = await stoGetUser(trade.sellerId);
  const buyer = await stoGetUser(trade.buyerId);
  if (!seller) return i.editReply('Trade failed: seller profile not found.');
  if (!buyer) return i.editReply('Trade failed: buyer profile not found. Buyer should use any bot command once, then try again.');

  const price = stoBig(trade.price);

  const removed = await stoRemoveTokens(trade.buyerId, price);
  if (!removed.ok) {
    if (removed.reason === 'funds') return i.editReply(`Buyer does not have enough Tokens. Need **${stoMoney(price)}**, has **${stoMoney(removed.current || 0)}**.`);
    return i.editReply(`Trade failed: could not remove Tokens from buyer. Reason: **${removed.reason}**`);
  }

  const added = await stoAddTokens(trade.sellerId, price);
  if (!added) {
    await stoAddTokens(trade.buyerId, price).catch(() => null);
    return i.editReply('Trade failed: could not add Tokens to seller. Buyer refund attempted.');
  }
  ...
  if (!moved) {
    await stoAddTokens(trade.buyerId, price).catch(() => null);
    await stoRemoveTokens(trade.sellerId, price).catch(() => null);
    return i.editReply('Trade failed: could not move character. Token rollback attempted.');
  }
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `<@${trade.buyerId}> bought from <@${trade.sellerId}>:\n\n` +
      `${stoCardLine(card)}\n\n` +
      `Price paid: **${stoMoney(price)} Tokens**\n` +
      `Buyer Tokens left: **${stoMoney(removed.after)}**`
    )
    .setColor(0x2ecc71);
  if (c.imageUrl) embed.setImage(c.imageUrl);
...
async function stoAccept(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);
  const tradeId = i.options.getString('trade_id', true);
  return stoComplete(i, tradeId);
}
async function stoCancel(i, tradeIdInput = null) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);
...
async function stoTrades(i) {
  if (!i.deferred && !i.replied) await i.deferReply().catch(() => null);

  const rows = [];
  for (const [id, trade] of safeTradeMemory.entries()) {
    if (stoExpired(trade)) {
      safeTradeMemory.delete(id);
      continue;
    }
    if (String(trade.buyerId) !== String(i.user.id) && String(trade.sellerId) !== String(i.user.id)) continue;
    ...
    rows.push(
      `ID: \`${id}\`\n` +
      `Seller: <@${trade.sellerId}> | Buyer: <@${trade.buyerId}>\n` +
      `Character: **${card?.character ? stoClean(card.character.name) : 'Unknown'}**\n` +
      `Price: **${stoMoney(trade.price)} Tokens**\n` +
      `Expires: <t:${Math.floor(Number(trade.expiresAt) / 1000)}:R>`
    );
  }
...
async function stoButtons(i) {
  if (!i.isButton()) return false;

  if (i.customId.startsWith('sto_accept_')) {
    await i.deferReply().catch(() => null);
    return stoComplete(i, i.customId.replace('sto_accept_', ''));
  }
  if (i.customId.startsWith('sto_decline_')) {
    await i.deferReply().catch(() => null);
    return stoCancel(i, i.customId.replace('sto_decline_', ''));
  }

  return false;
}
async function stoHandler(i, userId, commandName) {
  if (commandName === 'trade-offer') return stoTradeOffer(i);
  if (commandName === 'trade-accept') return stoAccept(i);
  if (commandName === 'trade-decline') return stoCancel(i);
  if (commandName === 'trade-cancel') return stoCancel(i);
  if (commandName === 'trades') return stoTrades(i);
  return false;
}
// ===== END TRADE ONLY FINAL SAFE PATCH =====

client.on('interactionCreate', async (i) => {
    const stoButtonHandled = await stoButtons(i);
    if (stoButtonHandled !== false) return stoButtonHandled;

    const ftButtonHandled = await ftButtons(i);
    if (ftButtonHandled !== false) return ftButtonHandled;
    ...
    if (i.isAutocomplete()) {
      const bpnAuto = await bpnAutocomplete(i);
      if (bpnAuto) return;
    }
  ...
  try {
    ...
    if (i.isAutocomplete()) {
      const ulAuto = await ulPackAutocomplete(i);
      if (ulAuto) return;
      return;
    }
    ...
    const stoHandled = await stoHandler(i, userId, commandName);
    if (stoHandled !== false) return stoHandled;

    const ftHandled = await ftHandler(i, userId, commandName);
    if (ftHandled !== false) return ftHandled;
    ...
    const bpnHandled = await bpnHandler(i, userId, commandName);
    if (bpnHandled !== false) return bpnHandled;
