...
client.once('ready', () => {
  ...
  ensureItemTemplates(prisma).then(() => console.log('Item templates synced')).catch(console.error);
  ...
  try {
    ...
    if (i.commandName === 'roll') {
      ...
      if (type === 'item') {
        ...
        return i.editReply(itemReplyPayload(r, '⚔️ Item Roll!', `🎲 Rolls left: **${user.rolls - 1}**`));
      }
    ...
    if (i.commandName === 'pack') {
      ...
      if (r.item) return i.editReply(itemReplyPayload(r, `📦 ${type.toUpperCase()} Pack`));
